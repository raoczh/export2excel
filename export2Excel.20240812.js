...
function create_json_to_excel_ws({
  fieldMap = {},
  sourceData = [],
  autoWidth = true,
  mergeColumns = [],
...
function export_json_to_excel(
  ...
  const sheets = [
    {
      name: 'SheetJS',
      data: { fieldMap, sourceData, autoWidth, mergeColumns, mainMergeColumns },
    },
  ]
  export_json_to_excel_multi_sheet(
    {
      sheets,
      bookType,
      filename,
...
/**
 *
 * @param {Object} param0 参数：{sheet：表头字段映射，sourceData：数据源，filename：文件名，mergeColumns：要自动合并的列名（自动合并连续相同的行数据）}
 * @param {*} callback 回调函数
 */
function export_json_to_excel_multi_sheet(
  { sheets = [], bookType = 'xlsx', filename } = {},
  callback
) {
  ...
  for (let i = 0; i < sheets.length; i++) {
    ...
    var ws = create_json_to_excel_ws(data)
